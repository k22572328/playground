...
// newRigged 開一局用未洗牌牌堆發的遊戲。rules.NewDeck 由小到大排列，
...
// card 簡寫一張牌。
func card(r rules.Rank, s rules.Suit) rules.Card { return rules.Card{Rank: r, Suit: s} }

// setHands 直接指定每位玩家的手牌，用來擺出特定局面。
func setHands(m *Match, hands [NumPlayers][]rules.Card) {
...
func mustPlay(t *testing.T, m *Match, seat int, cards []rules.Card) {
	t.Helper()
...
func openWithClubThree(t *testing.T, m *Match) {
	t.Helper()
	mustPlay(t, m, 0, []rules.Card{rules.ClubThree})
}

func TestOpeningMustContainClubThree(t *testing.T) {
	m := newRigged(t)
	if err := m.Play(0, []rules.Card{card(rules.Three, rules.Diamonds)}); err != ErrMustPlayClub3 {
		t.Errorf("出不含梅花 3 的第一手，err = %v, 想要 %v", err, ErrMustPlayClub3)
	}
	if err := m.Play(0, []rules.Card{rules.ClubThree}); err != nil {
...
func TestNotYourTurn(t *testing.T) {
	m := newRigged(t)
	if err := m.Play(1, []rules.Card{card(rules.Three, rules.Diamonds)}); err != ErrNotYourTurn {
...
func TestCannotPlayCardsYouDontHold(t *testing.T) {
	...
	if err := m.Play(1, []rules.Card{card(rules.Two, rules.Spades)}); err != ErrNotYourCards {
...
func TestSuitBreaksTie(t *testing.T) {
	...
	setHands(m, [NumPlayers][]rules.Card{
		...
		1: {card(rules.Three, rules.Diamonds), card(rules.Nine, rules.Diamonds)},
		2: {card(rules.Four, rules.Clubs), card(rules.Ten, rules.Clubs)},
		3: {card(rules.Five, rules.Clubs), card(rules.Jack, rules.Clubs)},
	})
	openWithClubThree(t, m)
	mustPlay(t, m, 1, []rules.Card{card(rules.Three, rules.Diamonds)})
	mustPlay(t, m, 2, []rules.Card{card(rules.Four, rules.Clubs)})
}
...
func TestTooSmallRejected(t *testing.T) {
	...
	setHands(m, [NumPlayers][]rules.Card{
		0: {rules.ClubThree, card(rules.Nine, rules.Clubs)},
		1: {card(rules.Ace, rules.Spades), card(rules.Four, rules.Clubs)},
		2: {card(rules.Five, rules.Clubs), card(rules.Six, rules.Clubs)},
		3: {card(rules.Seven, rules.Clubs), card(rules.Eight, rules.Clubs)},
	})
	openWithClubThree(t, m)
	// 檯面是梅花 3，座位 1 想用更小的牌壓 —— 手上沒有比 3 小的，
	// 改測同點數但花色更小的情形：先讓座位 1 出黑桃 A 拉高檯面。
	mustPlay(t, m, 1, []rules.Card{card(rules.Ace, rules.Spades)})
	// 檯面是黑桃 A，座位 2 的梅花 5 壓不過。
	if err := m.Play(2, []rules.Card{card(rules.Five, rules.Clubs)}); err != ErrTooSmall {
...
func TestWrongComboSizeRejected(t *testing.T) {
	...
	setHands(m, [NumPlayers][]rules.Card{
		0: {rules.ClubThree, card(rules.Nine, rules.Clubs)},
		1: {card(rules.Four, rules.Clubs), card(rules.Four, rules.Diamonds), card(rules.Nine, rules.Diamonds)},
		2: {card(rules.Five, rules.Clubs), card(rules.Six, rules.Clubs)},
		3: {card(rules.Seven, rules.Clubs), card(rules.Eight, rules.Clubs)},
	})
	openWithClubThree(t, m)
	if err := m.Play(1, []rules.Card{card(rules.Four, rules.Clubs), card(rules.Four, rules.Diamonds)}); err != ErrTooSmall {
...
func TestPassForfeitsRound(t *testing.T) {
	...
	setHands(m, [NumPlayers][]rules.Card{
		...
		1: {card(rules.Two, rules.Clubs), card(rules.Two, rules.Diamonds),
			card(rules.Two, rules.Hearts), card(rules.Two, rules.Spades), card(rules.Four, rules.Clubs)},
		2: {card(rules.Jack, rules.Clubs), card(rules.Jack, rules.Diamonds),
			card(rules.Ten, rules.Clubs)},
		3: {card(rules.Three, rules.Diamonds), card(rules.Three, rules.Hearts),
			card(rules.Three, rules.Spades), card(rules.Five, rules.Clubs), card(rules.Six, rules.Clubs)},
	})
	openWithClubThree(t, m)
	...
	mustPlay(t, m, 2, []rules.Card{card(rules.Jack, rules.Clubs), card(rules.Jack, rules.Diamonds)})
	mustPlay(t, m, 3, nil)
...
func TestPassStateClearsNextRound(t *testing.T) {
	m := newRigged(t)
	setHands(m, [NumPlayers][]rules.Card{
		0: {rules.ClubThree, card(rules.Four, rules.Clubs)},
		1: {card(rules.Ace, rules.Spades), card(rules.Five, rules.Clubs)},
		2: {card(rules.Six, rules.Clubs)},
		3: {card(rules.Seven, rules.Clubs)},
	})
	openWithClubThree(t, m)
	...
	mustPlay(t, m, 0, []rules.Card{card(rules.Four, rules.Clubs)})
	if m.Turn != 1 {
		t.Fatalf("新 Round 應輪到座位 1，實際 %d", m.Turn)
	}
	mustPlay(t, m, 1, []rules.Card{card(rules.Five, rules.Clubs)})
}
...
func TestRankingAndSuccession(t *testing.T) {
	...
	mustPlay(t, m, 1, []rules.Card{card(rules.Nine, rules.Clubs)})
}
...
func TestFourthPlaceDecidedAutomatically(t *testing.T) {
	...
	setHands(m, [NumPlayers][]rules.Card{
		...
		2: {card(rules.Five, rules.Clubs)},
		// 座位 3 還剩一堆牌，但不必打完。
		3: {card(rules.Six, rules.Clubs), card(rules.Seven, rules.Clubs), card(rules.Eight, rules.Clubs)},
	})
	openWithClubThree(t, m)                                        // 座位 0 第一名
	mustPlay(t, m, 1, []rules.Card{card(rules.Four, rules.Clubs)}) // 座位 1 第二名
	mustPlay(t, m, 2, []rules.Card{card(rules.Five, rules.Clubs)}) // 座位 2 第三名
	...
	if err := m.Play(3, []rules.Card{card(rules.Six, rules.Clubs)}); err != ErrMatchOver {
...
func TestLegalMovesAreAllPlayable(t *testing.T) {
	...
	for _, mv := range moves {
		// 開局的每一組都必須含梅花 3。
		if !mv.Contains(rules.ClubThree) {
